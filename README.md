...
| :--- | :--- | :--- | :--- |
| **`addRule(id, ruleFn)`** | `id` (String): A unique identifier for the rule.\<br\>`ruleFn` (Function): The function to execute. | `void` | Adds a function to the dynamic content engine. The `ruleFn` will be executed immediately and whenever a relevant DOM mutation is observed. |
| **`removeRule(id)`** | `id` (String): The identifier of the rule to remove. | `void` | Removes a function from the dynamic content engine, stopping its execution on future DOM mutations. |
...
| **`_cobaltApiCall(videoUrl, audio, quality, format)`** | `videoUrl` (String): The URL of the YouTube video.\<br\>`audio` (Boolean): Whether to request audio only.\<br\>`quality` (String): The desired video quality (e.g., '1080').\<br\>`format` (String): The desired container format (e.g., 'webm'). | `Promise<String\|null>` | A private function within the Cobalt feature that makes a POST request to the Cobalt API to get a direct download link for a video. |
